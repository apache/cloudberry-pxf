...
The Markdown source under [`content/`](content/) is published as part of the
Apache Cloudberry website at <https://cloudberry.apache.org/docs>. The site
build is owned by [`apache/cloudberry-site`](https://github.com/apache/cloudberry-site)
...
- Internal links use relative `.md` paths (for example,
  `./hdfs_text.md` or `../administering/cfg_server.md#about-the-pxf-fs-basepath-property`).
  Heading anchors are auto-generated by Docusaurus from the heading text.
- Images live under [`content/graphics/`](content/graphics/) and are referenced
  with relative paths (for example, `../graphics/pxfarch.png`).
