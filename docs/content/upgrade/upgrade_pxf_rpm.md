---
title: Upgrading PXF
description: Upgrading PXF via RPM.
sidebar_position: 4
---

If you have installed the PXF `rpm` or `deb` package and have initialized, configured, and are using PXF in your current Apache Cloudberry 5.21.2+ or 6.x installation, you must perform some upgrade actions when you install a new version of PXF.
...
1. Log in to the Apache Cloudberry master host. For example:

    ``` shell
    $ ssh gpadmin@<gpmaster>
    ```

2. Identify and note the version of PXF currently running in your Apache Cloudberry cluster:

    ``` shell
    gpadmin@gpmaster$ pxf version
    ```

2. If the `$GPHOME/pxf` directory exists, and you are running PXF version 5.12.x or older, back up the Apache Cloudberry PXF embedded installation. For example:
...
2. Stop PXF on each segment host as described in [Stopping PXF](../administering/cfginitstart_pxf.md#stopping-pxf).

3. Install the new version of PXF, identify and note the new PXF version number, and then continue your PXF upgrade with [Step 2: Completing the PXF Upgrade](#step-2-upgrade-pxf).
...
4. Synchronize the PXF configuration from the master host to the standby master host and each Apache Cloudberry segment host. For example:

    ``` shell
    gpadmin@gpmaster$ pxf cluster sync
    ```
 
5. Start PXF on each segment host as described in [Starting PXF](../administering/cfginitstart_pxf.md#starting-pxf).
