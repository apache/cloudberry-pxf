...
9. If the `hive.server2.authentication` property in `hive-site.xml` is set to `KERBEROS`:
    ...
    2. Ensure that you have configured Kerberos authentication for PXF as described in [Configuring PXF for Secure HDFS](./pxf_kerbhdfs.md), and that you have specified the Kerberos principal and keytab in the `pxf-site.xml` properties as described in the procedure.
