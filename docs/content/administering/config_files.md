...
| [pxf.max.threads](./cfg_mem.md#another-option-for-resource-constrained-pxf-segment-hosts) | The maximum number of PXF tomcat threads. | 200 |
...
| [pxf.log.level](./cfg_logging.md) | The log level for the PXF Service. | info  |
...
| [server.address](./cfghostport.md) | The PXF server listen address. | localhost |
...
| [PXF_LOGDIR](./cfg_logging.md#configuring-the-log-directory)   | The PXF log directory. | $PXF_BASE/logs |
| PXF_RUNDIR   | The PXF run directory. | $PXF_BASE/run |
| [PXF_JVM_OPTS](./cfg_mem.md#increasing-the-jvm-memory-for-pxf)  | The default options for the PXF Java virtual machine. | -Xmx2g -Xms1g |
| [PXF_OOM_KILL](./cfg_mem.md#configuring-out-of-memory-condition-actions)  | Activate/deactivate PXF auto-termination on OutOfMemoryError (OOM). | true (activated) |
| [PXF_OOM_DUMP_PATH](./cfg_mem.md#configuring-out-of-memory-condition-actions)  | The absolute path to the dump file that PXF generates on OOM. | No dump file (empty) |
...
The [Logging](./cfg_logging.md) advanced configuration topic describes how to enable more verbose client-level and server-level logging for PXF.
