...
If you choose the alternate deployment topology, you must explicitly configure each Apache Cloudberry host to identify the host and listen address on which the PXF Service is running. These procedures are described in [Configuring the Host](./cfghostport.md#configuring-the-host) and [Configuring the Listen Address](./cfghostport.md#configuring-the-listen-address).
